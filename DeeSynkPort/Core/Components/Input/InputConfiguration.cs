...
namespace DeeSynk.Core.Components.Input
{
    ...
    public struct MouseArgs
    {
        ...
        public Vector2 ScrollValue { get => _scrollValue; }

        Vector2 _scrollDelta;
        public Vector2 ScrollDelta { get => _scrollDelta; }

        long _time;
        public long Time { get => _time; }

        public MouseArgs(bool rawMouse, float dx, float dy, float x, float y, Vector2 scroll, Vector2 dScroll, long time)
        {
            _rawMouse = rawMouse;
            _dx = dx;
            _dy = dy;
    ...
    public class InputAssignment
    {
        ...
        public InputAssignment(InputType type, Keys key)
        {
        ...
        public InputAssignment(InputType type, Keys key, MouseButton button)
        {
            if (type == InputType.Keyboard)
            {
                _inputType = type;
